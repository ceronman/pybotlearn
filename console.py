...
# Code mostly stolen from Gedit's Python Console: http://www.gedit.org
 	...
 	def __run(self, command):
		...
		try:
			try:
				...
				if r is not None:
					if callable(r):
						warning =  _('It seems that you are trying to call a function,\\n')
						warning += _('but you are forgeting the parenthesis.\\n')
						warning += _('Try with: %s') % command.strip() + '()'
						self.eval('warning("%s")'% warning)
					else:
						print `r`
			except SyntaxError:
